...
#### 已知设计债（2026-09-04 记录，未修）

守卫的 `maxPerKey = 2` 仍然是**并发上限**，而不是**发起节流**：

```go
if len(group.leases) < m.maxPerKey && len(group.waiters) == 0 {
```

按实测这是错的 —— 6 个长时间持有的读取、间隔 300ms 发起，115 全部接受（6 × 206）。正确的设计应是"错开发起、不限在途"。

暂未修改的原因，均为事实而非偏好：

1. **当前没有造成失败**：指标 `slot_waits_total 11`、`slot_wait_timeouts_total 0`，只排队未超时；
2. **单读取者根本碰不到**：nginx slice 模块对单个下游请求是串行取片，在途恒为 1；
3. 上限只在同一文件有多个服务端读取者时才生效（两个房间看同一集且都在转码）；
4. 改造需重写 4 个围绕"等待自然释放"语义的并发测试，而该组件 gate 住全部转码播放。

修的时候应当：`maxPerKey` 降级为纯资源上限（如 8），新增 `startGap`（约 300ms）在锁内预约发起时刻，使同时到达者被排成等间隔序列；并重写 `TestThirdStreamWaitsForNaturalReleaseWithoutEviction` 等 4 个测试。
