...
#### 为什么服务端读取仍然经过守卫（2026-09-04 三）

吞吐修好后重新验证了"能直连的就直连"这条原则是否也该适用于服务端读取。结论是：**不该**，而且理由与之前完全不同。
...
|---|---|
| 直连（摘掉守卫钩子） | 4s / 4s / 2s |
| 经守卫（8m + keepalive） | 3s / 2s / 3s |

速度上已无差别 —— 守卫不再是成本。差别在突发承受能力：
...
ffmpeg 自身没有重试，403 即致命；而 Emby 的转码恢复机制（10 秒杀掉、立即重启）**恰好制造这种突发模式**。守卫的 FIFO 排队 + 刷新链接重试正好吸收它。

所以当前的边界是：

- **客户端**：一律直连，不经守卫（人数少，且插件的 `ProviderBurstPacer` 已错开服务端下发的命令）
- **服务端读取**（转码输入）：经守卫，理由是突发吸收，**不是**限制连接数
...
| 场景 | 结果 |
|---|---|
| 6 个长时间持有的读取、间隔 300ms 发起 | 6 × 206 |
| 3 个同一瞬间发起 | 2 × 206，1 × 403 |

所以正确的形状是**错开发起**，而不是限制在途。

现在的语义：

1. 调用者在锁内**预约发起时刻**，同时到达者被排成等间隔序列（间隔 300ms）；
2. **不与任何读取重叠的调用者不被延迟** —— 单个顺序读取者（转码就是）不可能与谁同时发起，节流它只会损失吞吐；
3. `maxPerKey` 降级为纯资源上限（8），仅防失控循环，不是提供商限制；超出者仍按 FIFO 排队；
4. 轮次落在等待窗口之外时返回 503 且**不打开上游**，由 Nginx 做一次刷新链接的延迟重试。

生产验证：

| 场景 | 旧设计 | 新设计 |
|---|---|---|
| 4 并发读取 | 4 × 206（后 2 个排队等释放） | 4 × 206 |
| **6 并发读取** | 只允许 2 在途，其余串行等待 | **6 × 206 全部并行** |
| 真实 seek（1:21 / 11:00 / 19:30） | 3s / 2s / 3s | **4s / 2s / 1s** |

第 2 条是关键教训：最初实现对所有调用者一律节流，导致首次 seek 从 3 秒退化到 6 秒 —— 顺序读取的每个切片都白等 300ms。只在真正重叠时才节流后恢复正常。

对应的 4 个并发测试（原本围绕"等待自然释放"语义）已按新模型重写，而非删除：
`TestConcurrentStreamsStartSpacedWithoutEviction`、
`TestSimultaneousAcquisitionsAreSpacedAndBounded`、
`TestGrantedWaiterIsPacedOnlyWhileAReadOverlaps`、
`TestTurnBeyondTheWaitWindowIsRefusedWithoutOpeningUpstream`。

#### 更正（2026-09-04 二）：守卫的吞吐才是转码失败的根因

现象是 4 个房间里只有《小林家的龙女仆》能播。它是唯一不需要转码的（MP4 / H.264 High / 无外挂字幕）；另外三个都是 MKV + HEVC + 外挂 ASS，必须转码。

日志显示 ffmpeg 每次都精确在 **10.0 秒**被 `SIGKILL`（退出码 137），Emby 立即重试，每次重试对 115 发起新请求，撞上"每瞬间 2 个新请求"的限制而得到 403，形成风暴。403 是**症状**，不是原因。

实测定位（同一文件、同一区间、未缓存）：

| 路径 | 24 MB 连续读 | 吞吐 |
|---|---|---|
| 经守卫 `/d/` | 14.5 秒 | 1.74 MB/s ≈ **13.9 Mbps** |
| 直连 115 CDN | 1.9 秒 | 12.91 MB/s ≈ 103 Mbps |

守卫的吞吐**低于该文件 20 Mbps 的码率**，转码必然饿死。根因是 Go 守卫的 HTTP 客户端设了 `DisableKeepAlives: true`，每个 1 MiB 切片都要重建 TCP + TLS 握手；切片越小，握手开销占比越高。

两项修复叠加：

| 配置 | 吞吐 |
|---|---|
| 1 MiB 切片 + 无 keepalive（原始） | 1.74 MB/s |
| 1 MiB 切片 + keepalive | 3.30 MB/s |
| 8 MiB 切片 + 无 keepalive | 3.97 MB/s |
| **8 MiB 切片 + keepalive** | **10.41 MB/s** |

真实 seek 场景从 12 秒降到 2–3 秒，Emby 10 秒截止线有 3 倍余量。

连接复用与"限制瞬时新请求"的结论是同向的：复用连接**减少**新建请求的频率。原设计禁用 keepalive，是"限制并发连接数"那个错误前提的又一处残留。

切片大小和连接复用因此是**服务端读取的正确性约束**，不是可随意调的性能参数。健康检查中写死的 `slice 1m;` 标记已改为尺寸无关，否则调参会让检查误判成安装损坏并循环重装。
...
| 相信外挂字幕的扩展名 | Emby 按扩展名判 `Codec`，Emby Web 只对 `ass`/`ssa` 启用 libass（SubtitlesOctopus）。`.ass` 里装 SRT → libass `Failed to start a track` → worker 抛错 → 被当成 `mediadecodeerror` → 反复重启流 → 「当前没有兼容的流」 | 按内容判定真实格式并改正扩展名。剑风传奇 13 个已改，回滚映射 `/config/watchparty-backups/subtitle-rename-20260904-214744.map`。**原先此处写的「37187 个 `.ass` 里有 10041 个其实是 SRT」经 2026-09-05 复核无法复现，勿照用**——见下一行 |
| 用「读固定字节 + 依次试解码」判定字幕编码 | 固定读 8192 字节会把一个 UTF-8 字符截断 → `utf-8` 解码失败 → fallback 到 `utf-16`，而 utf-16 对几乎任何偶数长度字节流都「解码成功」，吐出乱码，于是真 ASS 被判成「未知」。2026-09-05 全库扫 38201 个 `.ass`/`.ssa` 得到 31642 真 ASS / 139 SRT / 6420「未知」，抽样的「未知」全是被这个 bug 弄坏的真 ASS：`卛牣灩⁴湉潦` 按字节交换回来正是 `[Script Info]`。真实 SRT 数量只能界定在 139–6559 之间，与 10041 对不上 | ASS 标记和 SRT 时间码都是纯 ASCII，在 UTF-8/GBK/Big5 里字节不变，直接按 `latin-1` 解码找标记即可；只有 UTF-16 要按 BOM 或 NUL 密度单独识别。重命名前务必先干跑并核对总数 |
| 把恢复延时当成设备的单一属性 | 实测同一台 iPhone 一晚上的样本是 530/653/991/1351/1446/1709/1990/2630ms —— 两个簇而不是一个总体，取决于该片是直连还是要重启转码。单一标量对哪个簇都不准 | 按**房间**记忆（房间绑定内容，天然分开这两簇） |
| 参与者离开就丢弃学到的延时 | 换房间必然触发"参与者离开"，于是每进一个新房间第一次恢复的补偿是 `0ms`，完全没补偿 —— 而换房间正是人最常按播放的时刻 | `ClearSession` 只清在途观测，学习结果保留 |
| 首个样本也和先验做 EWMA 混合 | 从 0 起步的 α=0.5，第一次只补 50%、第二次 75%，每个房间前两次系统性欠补偿 | 房间无历史时首个观测直接采信；新房间用同一会话在其它房间的均值播种 |
